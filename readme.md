...
- Add the desired version of uci-apis which you want to use to `.env`. Run `setup-manual.sh` and follow the prompts.
...
- Some of the components are required to be created manually, they will be automated once once-click-deployment is completed
    ...
    - Org and ownerIDs to should be created
...
- **NOTE**: 
    ...
    - In server deployments, it is necessary to expose the fusionauth service on a specific port. It is recommended to associate a domain name with this service (for example: auth.yourdomain.com). Subsequently, you should update the `MINIO_IDENTITY_OPENID_CONFIG_URL` configuration setting by replacing `fusionauth` with either the server's IP address along with the designated port or the domain name you have directed it to. For running it on your local machine, the provided configurations should suffice.
## Docker Setup Steps
